	...
	$.getJSON("data/FAR_Accident_fatalities_year_state_geojson.json")
		.done(function(data) {
			$.getJSON("data/ne_50m_admin_1_states_provinces_lakes.json").done(function(choroplethData){
				...
				passChoropleth(info.timestamps, choroplethData);
			})

		})
		.fail(function() { alert("There has been a problem loading the data.")});

	function passChoropleth(timestamps, choroplethData){
		var timestamp = timestamps[0];
		choropleth = L.geoJson(choroplethData, {
			onEachFeature: function(feature, layer){
				var	popupContent = "<b>" + String(feature.properties[timestamp]) + " traffic fatalities</b><br>" +
								   "<p> in " + feature.properties.name +
								   " in " + timestamp + "</b>";

				layer.bindPopup(popupContent, {offset: L.point(0, 10)});
				layer.setStyle(style(feature.properties, timestamp))
			}
		});
...
//add listener on layer control for base layer change event and then create
//a function using jquery to modify the legend

		//style(choroplethData);
		...
		// 	{
		// 		return f.properties[choroplethData];
		// 	});
		// 	return false;
		//}
	}
	...
	function style(props, timestamp) {
	    return {
	        fillColor: getColor(props[timestamp]),
	        weight: 2,
	        opacity: 1,
	        color: 'white',
	        dashArray: '3',
...
function processChoroplethData(choroplethData) {
		...
		for (var feature in choroplethData.features) {

			var properties = choroplethData.features[feature].properties;
	...
	function updatePropSymbols(timestamp) {
		...
		if (choropleth){
			choropleth.eachLayer(function(layer) {
				...
				layer.setStyle(style(props, timestamp))
			});
		}
